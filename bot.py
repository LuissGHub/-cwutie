...
class EmbedModal(discord.ui.Modal, title="Create Embed"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Discord requires at least one visible field — use invisible char as fallback
        if not title_val and not desc_val and not image_val:
            desc_val = "\u200b"  # zero-width space, invisible to users
        ...
        if self.save_name and guild_id:
            ...
            conn = get_db()
            cur = conn.cursor()
            # Check if name already exists and this is a NEW embed (not an edit)
            cur.execute(
            ...
            if existing:
                cur.execute(
                    ...
                    (title_val, desc_val, theme_val, image_val,
                     thumb_val, int(self.use_avatar), now, guild_id, self.save_name),
                )
            else:
                cur.execute(
                    """INSERT INTO saved_embeds
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (guild_id, self.save_name, title_val, desc_val, theme_val,
                     image_val, thumb_val, int(self.use_avatar), now, now),
                )
            conn.commit()
            conn.close()
            if self.post_here:
            ...
            # Just post directly to current channel (Mimu style)
            await interaction.response.send_message(embed=embed)
...
async def on_ready():
    ...
    # Always sync globally so commands exist everywhere
    synced = await bot.tree.sync()
    print(f"Synced {len(synced)} global command(s)")
    # Also sync to guild for instant updates if GUILD_ID is set
    if GUILD_ID:
        try:
...
async def embedpost(interaction: discord.Interaction, name: str):
    ...
    if row is None:
        await interaction.response.send_message(
            f"No embed named **{name}**. Use `/embedlist` to see all saved embeds.", ephemeral=True
        )
        return
    embed = build_embed(
    ...
    # Defer ephemerally so the "used /embedpost" line is hidden, then send embed to channel
